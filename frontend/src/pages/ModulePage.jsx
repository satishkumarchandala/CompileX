...
import {
  ...
  LinearProgress, Alert, Tabs, Tab, Chip
} from '@mui/material'
import { Quiz, VideoLibrary, Description, PlayCircleOutline, OpenInNew, CheckCircle } from '@mui/icons-material'
import { getModule, getProfile } from '../api'
      ...
      <Paper elevation={3} sx={{ p: 4, mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', mt: 4 }}>
          <Button
            ...
            size="large"
            startIcon={isCompleted ? <CheckCircle /> : <Quiz />}
            color={isCompleted ? 'success' : 'primary'}
            onClick={() => navigate(`/module/${id}/quiz`)}
            sx={{ px: 6, py: 1.5 }}
          >
            {isCompleted ? 'Review Quiz (No XP)' : 'Take Quiz'}
          </Button>
        </Box>
      </Paper>
    </Container>
